		...
		module V1
			StockItemsController.class_eval do
				...
				def index
					...
					stock_items = Spree::StockLocation.find(params[:stock_location_id]).stock_items
					...
					@stock_items = stock_items.where(variant: variants, count_on_hand: 1..Float::INFINITY).page(params[:page]).per(params[:per_page])
