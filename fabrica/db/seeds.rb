...
formulas_csv.each do |formula_attrs|
  ...
  variante = Spree::Variant.find_by_sku(sku)
  variante.lote_minimo = formula_attrs['Lote']
  variante.save
  if formula_attrs['Manzana'].to_i > 0
    Recipe.new({
      sku: sku, ingredient_variant_sku: "20",
      amount: formula_attrs['Manzana'].to_i
      }
    ).save
  end
  if formula_attrs['Naranja'].to_i > 0
    Recipe.new({
      sku: sku, ingredient_variant_sku: "30",
      amount: formula_attrs['Naranja'].to_i
      }
    ).save
  end
  if formula_attrs['Frutilla'].to_i > 0
    Recipe.new({
      sku: sku, ingredient_variant_sku: "40",
      amount: formula_attrs['Frutilla'].to_i
      }
    ).save
  end
  if formula_attrs['Frambuesa'].to_i > 0
    Recipe.new({
      sku: sku, ingredient_variant_sku: "50",
      amount: formula_attrs['Frambuesa'].to_i
      }
    ).save
  end
  if formula_attrs['Durazno'].to_i > 0
    Recipe.new({
      sku: sku, ingredient_variant_sku: "60",
      amount: formula_attrs['Durazno'].to_i
      }
    ).save
  end
  if formula_attrs['Arándano'].to_i > 0
    Recipe.new({
      sku: sku, ingredient_variant_sku: "70",
      amount: formula_attrs['Arándano'].to_i
      }
    ).save
  end
